...
class ClientDAO:
    ...
    def insertion_client_or_company(self, liste, bool):
        ...
        liste_c = liste[:6]
        bdd.cursor.execute(
            """INSERT INTO client(last_name,first_name,e_mail,address,phone,remark) VALUES(?,?,?,?,?,?)""", liste_c)
        ...
        if bool == 1:
            ...
            """ recuperation de id """
            bdd.cursor.execute(
                """INSERT INTO company(id_company,company_name,num_SIRET) VALUES(?,?,?)""", liste_e)
            bdd.connexion.commit()
    ...
    def delete_table(self, name, id):
        bdd.cursor.execute(""" DELETE FROM"""+name +
                           """ WHERE id_"""+name+"""="""+str(id))
        bdd.connexion.commit()
...
class UserDAO:
    ...
    def insertion_user(self, liste):
        ...
        bdd.cursor.execute(
            """INSERT INTO user(logo,company_name,e_mail,address,phone,num_SIREN) VALUES(?,?,?,?,?,?)""", liste)
        bdd.connexion.commit()
    ...
    def selection_table(self, nom):
        bdd = DBmanager.get_instance()
        bdd.cursor.execute("""select * frosm """+nom)
        return bdd.cursor.fetchall()
...
class DepositDAO():
    ...
    def insertion_deposit(self, liste):
        ...
        data = date.today()
        bdd.cursor.execute(
            """INSERT INTO deposit(date,amount,id_invoice) VALUES(?,?,?)""", (data, liste[0], liste[1]))
        bdd.connexion.commit()
    ...
    def update_deposit(self, liste):
        ...
        liste = liste[1:]+[liste[0]]
        bdd.cursor.execute(
            """UPDATE deposit SET date=?,amount=?, id_invoice=? WHERE id_deposit=?""", liste)
        bdd.connexion.commit()
...
class InvoiceDAO:
    ...
    def insertion_invoice(self, liste):
        bdd = DBmanager.get_instance()
        bdd.cursor.execute(
            """SELECT max(id_invoice_devis) FROM invoice_devis""")
        bdd.connexion.commit()
        id_max = bdd.cursor.fetchall()
        bdd.cursor.execute(
            """INSERT INTO invoice(id_invoice,solde) VALUES(?,?)""", (id_max[0][0], liste[0]))
        bdd.connexion.commit()
...
class ArticleDAO:
    ...
    def insertion_article(self, liste):
        bdd = DBmanager.get_instance()
        bdd.cursor.execute(
            """INSERT INTO article(name,description,price) VALUES(?,?,?)""", liste)
        bdd.connexion.commit()
    ...
    def update_article(self, liste):
        ...
        liste = liste[1:]+[liste[0]]
        bdd.cursor.execute(
            """UPDATE article SET name=?,description=?,price=? WHERE id_article=?""", liste)
        bdd.connexion.commit()
...
class Invoice_devDAO:
    ...
    def insertion_invoice_dev(self, liste):
        bdd = DBmanager.get_instance()
        bdd.cursor.execute(
            """INSERT INTO invoice_devis(amount,date,description,note,remark,id_client,id_user) VALUES(?,?,?,?,?,?,?)""", liste)
        bdd.connexion.commit()
    ...
    def update_invoice_dev(self, liste):
        ...
        liste = liste[1:]+[liste[0]]
        bdd.cursor.execute(
            """UPDATE invoice_devis SET amount=?,date=?,description=?,note=?,remark=?,id_client=?,id_user=? WHERE id_invoice_devis=?""", liste)
        bdd.connexion.commit()
...
class Art_devDAO:
    ...
    def insertion_art_dev(self, liste):
        ...
        bdd.cursor.execute(
            """INSERT INTO art_dev(id_article,id_invoice_devis) VALUES(?,?)""", (liste))
        bdd.connexion.commit()
