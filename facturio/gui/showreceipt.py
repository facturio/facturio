...
# from facturio.db.import AdvanceDAO
from datetime import datetime
import re
from datetime import date


class ShowReceiptPage(Gtk.ScrolledWindow):
    ...
    def get_instance():
        """Renvoie l'instance."""
        if ShowReceiptPage.__instance is None:
            ShowReceiptPage.__instance = ShowReceiptPage()
        return ShowReceiptPage.__instance
    ...
    def __init__(self):
        ...
        self.adv_sep = Gtk.HSeparator()
        self.main_grid.attach(self.adv_sep, 2, 5, 1, 1)
    ...
    def _init_advances_grid(self):
        ...
        self.plus_btn_adv = Gtk.Button.new_from_icon_name("list-add-symbolic",
                                            Gtk.IconSize.BUTTON)
        ...
        self.plus_btn_adv.connect("clicked", self._new_adv_row)
        self.advances_grid.attach(self.plus_btn_adv, 1, 3, 1, 1)
    ...
    def _new_adv_row(self, *args):
        ...
        entry.connect("changed", self._reset_context)
        entry.show()
    ...
    def _init_header_grid(self):
        ...
        self.title = Gtk.Label("<big>" + i18n.t('gui.invoice') + "</big>")
        self.title.set_hexpand(True)
        self.title.set_use_markup(True)
        self.header_grid.attach(self.title, 1, 1, 4, 1)

    def _reset_context(self, entry):
    ...
    def _init_total_grid(self):
        ...
        self.total_grid.attach(self.space, 2, 1, 1, 1)
    ...
    def _init_taxes_grid(self):
        ...
        self.date_entry = self._grayed_entry()
        self.taxes_grid.attach(self.date_entry, 3, 2, 1, 1)
    ...
    def load_receipt(self, receipt, add_adv: bool = None):
        ...
        self.save_modifs_btn.hide()
        self.plus_btn_adv.hide()
        if type(receipt) == Invoice:
            self.adv_sep.show()
            self.advances_grid.show()
            self.adv_label.show()
            self.adv_total.show()
            ...
            self.adv_label.hide()
            self.adv_total.hide()
        self.date_entry.set_text(receipt.date_string())
